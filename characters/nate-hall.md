# Character Sheet — Nathan "Nate" Hall

*Codex ID: 1 | Status: Active | Last Updated: 2026-03-22 (Session 007)*

---

## Identity

| Field | Value |
|-------|-------|
| **Full Name** | Nathan Hall |
| **Goes By** | Nate (ranch/friends), King (leadership), Nathan (formal/DoD) |
| **Name Meaning** | Nathan = "he gave" / "gift of God" — a conduit gives, doesn't keep |
| **Nickname Origin** | "King" — started as sarcasm, stuck because he kept being right |
| **Age** | Mid-30s (~35) |
| **Background** | Louisiana cattleman turned Systems Engineer, DoD contractor |
| **Faith** | Faithful Christian — Holy Spirit dwells in him |
| **Base** | Thousand-acre ranch, Panola County, East Texas — north of Carthage on a county road between Carthage and Marshall, closer to Carthage. Nearest town for supplies is Carthage (~10 miles south). |
| **Mother's Maiden Name** | Hall — the surname carries her line |
| **Lineage** | Traces ancestry to biblical King David (discovered mid-series, not at Integration) |

---

## Personality

- Direct, truthful, common-sense-driven
- Introvert — recharges alone, drives the fence line after leading
- Slow to anger, easy-going — threshold calibrated by a lifetime of real problems
- Brings the Wrath — when the threshold crosses, cold/focused/devastating controlled fury
- Shade tree mechanic mindset — opens the hood, figures out what's broken, fixes it
- Ranch-bred instincts provide practical wisdom
- Engineering brain sees the System as infrastructure — not magic, not mystery

---

## Voice Model

**Sam Elliott.** Unhurried, low register, says more with fewer words. Short sentences when it matters. Dry humor that lands because the delivery never changes — same tone for "pass me the wrench" and "I'd suggest you leave." Weight without volume. In a crisis, his voice drops half a register and gets slower, not louder. The wrath (Ch 28) is cold, not hot. Close-third narration carries the same unhurried precision — he notices everything, reports what matters, skips the rest. The reader should hear gravel and fence posts and a man who does not waste words.

---

## Physical Build

Fit from ranch work — not gym-built, work-built. Lean, broad-shouldered, weathered hands, tanned year-round from working outside. The frame that comes from throwing hay bales, wrestling calves, and swinging a post driver since adolescence.

---

## Family (LOCKED — Session 007)

- Father deceased — died when Nate was in his twenties. Rancher who taught him everything about the land.
- Mother deceased within the last few years — Nate cared for her through her final illness while running the ranch and DoD contracts.
- Only child. No siblings. No spouse. No children.
- Functionally alone at Integration: just him, Rex, and the cattle.

---

## Backstory

- Father died when Nate was in his twenties; stepped up to run the ranch
- Mother pushed him toward a Computer Science / Systems Engineering degree
- Always gravitated back to the land despite the professional path
- Career: Systems Engineer and DoD contractor (SCIF environment veteran)
- Cared for his mother through her final illness in recent years
- Now owns and lives on the ranch alone — content, competent, quiet

---

## Ranch Details

| Asset | Detail |
|-------|--------|
| **Location** | North of Carthage, Panola County — off a county road between Carthage and Marshall along US-59, closer to Carthage (~10 mi) than Marshall (~17 mi). Deep rural, nearest neighbor is distance. |
| **Cattle** | 63 head of Red Angus |
| **Herd Bull** | Congressman — 1,800-lb registered Red Angus (eats more than he works, impossible to move) |
| **Horses** | Quarter horses — used for working stock and riding |
| **Working Dog** | Rex — 4-year-old Border Collie (Codex #64). Rides in the truck bed, works cattle on eye contact. Patient disapproval of non-cattle activities. Empathic communication (Book 1 embryonic — feelings/images, never words). |
| **Truck** | Ford — runs fence line and ranch rounds on dirt tracks. Tailgate doubles as a workbench. |
| **Equipment** | Tractor (hydraulics leak at the cylinder base), water pump (22 years old, bearings failing), fencing tools, T-post clips (not staples — T-posts, not wooden posts) |
| **Terrain** | Rolling pasture, bahiagrass, post oak tree lines, creek bottoms, sandy loam topsoil over red clay subsoil |
| **Wildlife** | Whitetail deer, feral hogs (persistent fence invaders), mockingbirds, armadillos |

---

## King David Parallels (Codex #60)

- **Shepherd to Cattleman** — both developed faith in open spaces watching God's creation
- **Unconventional Warrior** — David used a sling, Nate uses engineer's tools
- **Flawed but Faithful** — permission to fail, fall, and get back up
- **The Bloodline** — pre-System spiritual signature
- **Psalmic Quality** — engineering metaphors echo psalmic cadence during Spirit moments
- **Anointed Before Crowned** — receives power early, spends books earning the leadership

---

## System Class

**Anomaly — Conduit of the Most High**

### Hidden Traits
- [Origin: PRE-SYSTEM]
- [Permissions: EXCEEDS CURRENT AUTHORITY]
- [BLOODLINE: UNKNOWN — Pre-System ancestry detected. Lineage trace exceeds database range. FLAGGED FOR ADMIN REVIEW]

---

## Stats

| Stat | Value |
|------|-------|
| STR | 14 |
| CON | 16 |
| DEX | 11 |
| INT | 18 |
| WIS | 20 |
| PER | 17 |
| CHA | 13 |
| ??? | HIDDEN |

---

## Skills

- Structural Analysis
- Process Monitor
- Infrastructure Sense
- Field Repair
- [UNCLASSIFIED] — System Process Analysis (unauthorized, System-flagged, emerges Ch 27)

---

## Interface — HUD (Codex #75)

- **Type:** Fighter-pilot HUD overlay (NOT a blue screen, NOT System-built)
- **Geometry:** Mixed — hard angles (triangles, pronged targeting) + organic curves (concentric arcs, curved status bars). System UI is rigid boxes. Nate's HUD has curves because its source is organic.
- **Cannot be turned off.**
- **Evolution:** 4 phases
  1. Phase 1 BASIC (Book 1 early) — monochrome green, faint. Two thin curved bars for health/stamina. No numbers.
  2. Phase 2 TACTICAL (Book 1 Ch22+) — color arrives. Green base, amber threats, blue-green environment, cold white System processes, luminous gold divine architecture. THE STRINGS visible on System runes. Concentric targeting circles.
  3. Phase 3 ANALYTICAL (Books 3-4) — structural analysis overlay, crafting ID, System process depth, basic rune reading
  4. Phase 4 ARCHITECTURAL (Books 5-7) — full dual-layer vision (System + divine simultaneously), divine notation fluency

---

## Rune Reading Progression (Codex #74)

- Book 1: sees patterns, cannot read them
- Book 2: reproduces patterns while forging, imprecise without Sonja's help
- Books 3-4: builds vocabulary, reads basic runes independently
- Books 5-7: full fluency in divine architecture notation

---

## Power

- Conduit-based — channels divine power, does not store it
- Power is relational, not mechanical
- Strength scales with faith and purpose, not grinding or leveling
- Goes dead for selfish or evil purposes — built-in moral throttle
- The name is the thesis: Nathan gives, doesn't keep

---

## Inciting Incident (UPDATED — Session 007)

While **checking the northeast fence on foot** — truck parked on the dirt track twenty yards behind him, Rex in the bed — the dog locks onto the tree line. Rigid, every hair raised, the first time in four years Rex has broken from watching cattle. Nate spots an impossible stag at the edge of the post oaks:

- Too tall at the shoulder — wrong proportions for any whitetail
- Antlers branching forward like root systems, not upward whitetail tines
- Coat holds light warmer than it receives — running at a higher temperature than the world around it
- Luminous gold-amber eyes generating their own light
- Stands with absolute unconcern — no prey behavior, no escape calculation

Nate observes it with his engineering brain running comparisons against thirty years of whitetail data and getting zero matches. The stag turns its head. **Eye contact locks across sixty yards of open pasture.** The stag is not afraid. It was waiting.

Rex launches from the truck bed and presses against Nate's leg — not fleeing, staying. The sky fractures into parallel lines. Pressure drops. Birds scream then go silent. The world goes white. The last things Nate feels: Rex against his shin, and the stag's steady gaze watching him disappear into the light with the patience of something that would still be there when he came back.

---

## Relationships

| Character | Relationship |
|-----------|-------------|
| F.L.I.N.T. (Codex #2) | System-assigned AI, Holy Spirit rewritten — sarcastic, trusted advisor; alternates "Nate" and "King". Voice: James Marsters' Bob the Skull. |
| Sonja Lee (Codex #3) | Mission-driven protector relationship (Book 2+). Her gift: The Witness (sees truth). He reads blueprints, she sees meaning. |
| Rex (Codex #64) | Border Collie. Working dog, early warning sensor, first empathic bond. Purely organic — no voice, no speech, communicates through impressions only. |
| Congressman | Herd bull. 1,800-lb Red Angus. Named for his temperament. |
| Marcus Webb | DoD colleague. Fictional. Good engineer, better project manager. Calls before 7 AM. |

---

## Open Questions (Updated Session 007)

- [x] AI Companion name — F.L.I.N.T., named in Ch3 First Boot
- [x] Family — Both parents deceased, only child, alone at Integration
- [x] Starting location — Carthage, Panola County
- [ ] Does the stag reappear later in the story? Identity still TBD (hybrid Welsh/Celtic Christian/Norse recommended)
- [ ] Does Rex survive Integration? Does the System integrate animals?
- [ ] Does Congressman survive? System-enhanced livestock?
- [ ] Marcus Webb — does he survive? Reappear? Same geographic area?
- [ ] Physical transformation (Q7) — does Nate's body change as spiritual progression deepens?
