...
- **Skill taxonomy & acquisition (updated 2026-06-17)** — **Four-tier hierarchy:** Domain → Skill (main, ranked) → **Sub-skill** (ranked facet, e.g. Structural Analysis → Load-Path Read) → Ability (named move, e.g. Riposte). ("Sub-skill" is REINSTATED as a distinct ranked tier — this supersedes the old "sub-skill retired" rule.) Lowercase "technique" still OK as mundane prose flavor. **Two acquisition families:** **Granted** (System authors & pushes — class/level/patron/skill-book; popup *Class skill granted*; carries the reward fanfare) vs **Acquired** (System reads a capacity it didn't author — earned by doing: back-fill, Recognition Threshold cold-start, budding, graduation; popup *Skill acquired*; fanfare-free). Ten domains: 1-9 (Martial/Perception/Stealth/Social/Physical/Gathering/Crafting) + Domain 10 Life Skills (Home/Trade/Profession/Arts/Body/Mind/Social). See `feedback_skill_vs_ability_terminology.md` and `research/skill-system-design.md`.
...
- **Progression crunch (low-moderate)** — Stat blocks max ~6 lines. Skill choices: 2-3 options, decided same scene, no multi-chapter deliberation. Numbers shown *in play*, not audited on screens. Flint narrates all number-work with personality. No loadout optimization scenes. Level-ups at transitions, never mid-action. **Nate's HUD (refined 2026-06-17):** no persistent stat sheet and no stat-sheet recaps, BUT skill notifications (acquired / rank-up) DO surface in his HUD — **fanfare-free** (no System/MSG reward chime; the hook is the fanfare, not the notification), with on-demand skill detail + Flint rundown. Book 1 progression: mid-to-late-mid Book 1 rudimentary progress indicators; late Book 1+ Flint-authored HUD. See `project_forge_progression_and_flint_arc.md`.
